...
```{r load_packages_functions, warning=FALSE, message=FALSE}
...
# load the R1 - single read data first
r1_data <- load(here("_data","DO185_mESC_emase_m4_noProbs_correctedIDs_02012021.RData"))
esc_expr_r1 <- expr # cleaned, normalized, single read, R1
esc_raw_expr_r1 <- raw.expr # raw data, only filtered not normalized
rm(matches, exprNotes, mixupNotes, DO185_mESC_emase_m4.cmd, expr, raw.expr)

pe_data <- load(here("_data","DO185_mESC_paired_emase_m4_noProbs_correctedIDs_02012021.RData"))
esc_expr_pe <- expr # cleaned, normalized, paired end data
esc_raw_expr_pe <- raw.expr # only filtered not normalized
rm(matches, exprNotes, mixupNotes, DO185_mESC_emase_m4.cmd, expr, raw.expr)
...
esc_raw_expr_r1_tibble %>% 
  ...
  aes( y = PB366.18_repA, 
       x = PB357.12_repA) + # now we can set the aesthetics, what goes on which axes, what color... you can use variable/column names inside here.
...
esc_raw_expr_r1_tibble  %>% 
  ...
  aes( x = `PB366.18_repA`) + # aesthetics determine which variable is being plotted and how it is being used. you can use column names here. 
  geom_histogram( bins = 1000, col = "blue", alpha = 0.5) + # which plot to show
  geom_histogram( aes( x = `PB357.12_repA`), col = "red", bins = 1000, alpha = 0.4)+
  theme_pubclean() +
  scale_x_log10()
...
esc_raw_expr_r1_tibble  %>% 
  ...
  geom_density( aes( x = `PB357.12_repA`), col = "red",  alpha = 0.1, adjust =0.5)+
  theme_pubclean() +
  scale_x_log10()
...
Here is how this data have been processed so far. Note that we will do more before genetic mapping (batch correction using `comBAT` function from the `sva` package and rank normalization).

First, we filter genes that are expressed at very low levels using both the counts and the total reads per million (tpm) retrieved from EMASE.

**Question: What is tpm? What is the difference between the counts and tpm values we get from EMASE?**
...
Next, we do upper quartile normalization using the code below:
...
```{r mean_gene_expression_histogram_read1, fig.width=6, fig.height=4}
...
esc_raw_expr_r1_tibble_t %>% 
  ...
  rename( "mean" = "V1") -> esc_raw_r1_mean
