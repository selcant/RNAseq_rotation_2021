...
```
 ...
 ##Converting the data sets to tibbles and saving as new variables
...
pe_r1_means <- dplyr::bind_rows(list(pe_mean = esc_expr_pe_mean, r1_mean = esc_expr_r1_mean), .id = "dataset")
...
```{r}
esc_expr_pe_tibble %>% t() -> esc_expr_pe_tibble_t

esc_expr_pe_tibble_t %>% select(1:10000) %>% 
  ggplot() +
  aes( x = (esc_expr_pe_tibble_t [,1])) +
  geom_point( size = 2 ) +
  theme_pubclean() 
```
