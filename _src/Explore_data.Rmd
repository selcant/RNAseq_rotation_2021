...
```{r load_packages_functions, warning=FALSE}
...
esc_raw_expr_r1_tibble_t %>% 
  ...
  aes( x = `ENSMUSG00000000056`, 
       y = `ENSMUSG00000000028`) + # now we can set the aesthetics, what goes on which axes, what color... you can use variable/column names inside here.
  geom_point( size = 2 ) + # now we call the type of plot we want to see. in this case we want points. and set related parameters.
  theme_pubclean() # theme: visual properties of the plot.
...
```{r mean_gene_expression_histogram_read1, fig.width=8, fig.height=6}
